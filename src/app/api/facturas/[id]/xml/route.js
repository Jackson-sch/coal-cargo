import { NextResponse } from "next/server"; import { prisma } from "@/lib/prisma"; import { auth } from "@/lib/auth"; import ApiGoService from "@/lib/services/sunat/api-go"; export async function GET(request, { params }) { try { const session = await auth(); if (!session) { return NextResponse.json({ error: "No autorizado" }, { status: 401 }); }
    ...
    // Usar el servicio API-GO para descargar el XML const apiGoService = new ApiGoService(); const tipoDocumento = factura.tipoComprobante === "FACTURA" ? "factura" : "boleta"; const resultado = await apiGoService.descargarXML( factura.pseId, tipoDocumento ); if (!resultado.success) { const match = String(resultado.error || "").match( /\b(401|403|404|422|500)\b/ ); const status = match ? Number(match[1]) : 500; return NextResponse.json({ error: resultado.error }, { status }); }

    const nombreArchivo = `${factura.serie}-${factura.numero}.xml`; return new NextResponse(resultado.xml, { status: 200, headers: { "Content-Type": "application/xml", "Content-Disposition": `attachment; filename="${nombreArchivo}"`, "Cache-Control": "no-cache, no-store, must-revalidate", Pragma: "no-cache", Expires: "0", }, }); } catch (error) { console.error("Error al descargar XML:", error); return NextResponse.json( { error: "Error al descargar el XML" }, { status: 500 } ); }
}
